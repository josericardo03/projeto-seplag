...
export function PetInfoCard({ pet }: { pet: Pet }) {
  return (
    <div className="bg-white rounded-3xl p-6 shadow border border-slate-100 mb-8 grid grid-cols-1 sm:grid-cols-3 gap-6">
      <div>
        <span className="text-xs text-slate-500">Espécie</span>
        <p className="text-lg font-semibold text-slate-800">{pet.especie?.trim() ? pet.especie : '—'}</p>
      </div>
